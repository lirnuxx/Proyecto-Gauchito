...
runBtn.onclick = async () => {
    ...
    resultsContainer.innerHTML = "<p style='text-align:center;' class='titilar'>Analizando muestras de planchas...</p>";

    const formData = new FormData();
    formData.append('file', fileInput.files[0]);

    try {
        const response = await fetch('/analizar_fundido', { method: 'POST', body: formData });
...
function mostrarResumen(data) {
    ...
    lineas.forEach(linea => {
        ...
        // 🔹 BLOQUE APTAS
        if (texto.toUpperCase().startsWith("APTAS")) {
        ...
        // 🔹 BLOQUE NO APTAS
        else if (texto.toUpperCase().startsWith("MUESTRA")) {
            const match = texto.match(/Muestra\s+(\d+):\s*(.*)/i);
            if (match) {
                noAptas.push({
                    id: match[1],
                    motivo: match[2]
    ...
    if (aptas.length > 0) {
        ...
        divAptas.innerHTML = `<p><strong>Muestras APTAS:</strong> ${listaIds}</p>`;

        resultsContainer.appendChild(divAptas);
    }

    // Render NO APTAS
    noAptas.forEach(item => {
        const divNoApta = document.createElement('div');
        divNoApta.className = 'resultado-card-resumen no-apto';

        divNoApta.innerHTML = `
            <p><strong>Muestra ${item.id}:</strong> ${item.motivo}</p>
        `;

        resultsContainer.appendChild(divNoApta);
    });
}
